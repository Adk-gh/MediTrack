...
const formatDeptDisplay = (deptValue, deptOptions) => {
  ...
  if (Array.isArray(deptValue)) {
    ...
    try {
      ...
      arr = Array.isArray(parsed) ? parsed : [deptValue];
    } catch {
      arr = [deptValue];
    }
  }

  return arr.length === deptOptions.length &&
    deptOptions.every((d) => arr.includes(d))
    ? [ALL_DEPT_LABEL]
    : arr.length > 0
      ? arr
      : [ALL_DEPT_LABEL];
...
const ImageDropZone = ({ value, onChange, onClear }) => {
  ...
  const handleFiles = (files) => {
    ...
    if (file.size > 20 * 1024 * 1024) {
      return alert('Image must be smaller than 20 MB.');
    }
      ...
      {value ? (
        ...
        <div
          ...
          className={`border-2 border-dashed rounded-xl p-5 sm:p-6 text-center cursor-pointer transition-all ${
            drag
              ? 'border-[#466460] bg-[#e0eceb]/40'
              : 'border-[#d1e7e5] hover:border-[#466460] hover:bg-[#f0f7f6]'
          }`}
        >
          ...
          <p className="text-[11px] sm:text-[12px] text-slate-500 font-medium">
            ...
            <span className="text-[#466460] underline">
              browse
            </span>
          </p>
        </div>
...
export const Announcements = () => {
  ...
  const deptOptions =
    configData?.departments?.map((d) => d.full) || [];

  const getDeptAbbr = (fullName) =>
    configData?.departments?.find(
      (d) => d.full === fullName
    )?.abbr || fullName;
  ...
  const formDrawer = useDrawerDrag(() =>
  ...
  const handleDeleteConfirm =
    async () => {
      ...
      try {
        ...
        const response =
          await fetch(
            `${API_URL}/announcements/${announcementToDelete.id}`,
            {
              method: 'DELETE',
              headers: {
                Authorization: `Bearer ${token}`,
  ...
  const inputCls =
    'w-full mt-1.5 px-4 py-3 sm:py-3 border border-[#e2e8f0] rounded-xl text-[13px] sm:text-[14px] outline-none focus:border-[#466460] focus:ring-2 focus:ring-[#e0eceb] transition-all bg-white box-border';

  const labelCls =
    'block text-[10px] sm:text-[11px] font-bold text-slate-500 uppercase tracking-wide mt-5 sm:mt-6';
  ...
  return (
    <div className="bg-white h-[calc(100vh-120px)] flex flex-col animate-[fadeInSlide_0.4s_ease-out_forwards]">
          ...
          filtered.map((item) => {
            ...
            return (
              ...
              >
                ...
                <div
                  className={`flex-1 min-w-0 p-3 sm:p-4 relative flex flex-col justify-center ${
                    !item.image_url
                      ? 'ml-[3px] pl-4 sm:pl-5'
                      : 'pl-3 sm:pl-4'
                  }`}
                >
                  {canManage && (
                    <div
                      className="absolute top-2.5 right-2 sm:right-3"
                      onClick={(
                        e
                      ) =>
                        e.stopPropagation()
                  ...
                  <div className="flex flex-wrap items-center gap-2 mb-2 pr-8">
                    {item.priority !==
                      'normal' && (
                      <span
                        className={`text-[10px] font-bold px-2.5 py-1 rounded-full text-white ${pri.color} flex items-center gap-1`}
                      >
                        <i
                          className={`fa-solid ${
                            item.priority ===
                    ...
                    <span
                      className={`text-[11px] font-semibold px-2.5 py-1 rounded-full ${catCls}`}
                    >
                      {item.category ||
                        'General'}
                    </span>
          ...
          <div className="fixed inset-0 z-[99999] flex items-start justify-center pt-0 sm:pt-8">
            ...
            >
              <div
                ...
                onTouchMove={
                  formDrawer.handleTouchMove
                }
                onTouchEnd={
                  formDrawer.handleTouchEnd
                }
              >
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <div
                          id="dept-dropdown"
                          className="hidden absolute z-20 w-full mt-1 border border-[#e2e8f0] rounded-xl bg-white shadow-lg max-h-48 overflow-y-auto"
                        >
                          <label
                          ...
                          {deptOptions.map(
                            (d) => (
                              ...
                              >
                                <input
                                  type="checkbox"
                                  checked={formData.dept.includes(
                                    d
                                  )}
                                  onChange={(
                                    e
      ...
      {isViewModalOpen &&
        ...
        createPortal(
          <div
            className="fixed inset-0 z-[99999] flex justify-center items-end sm:items-center p-0 sm:p-6"
            onClick={() => {
              setIsViewModalOpen(
                false
              );
          ...
          >
            ...
            <div
              ...
              style={{
                transform:
                  viewDrawer.isDragging &&
                  viewDrawer.dragY > 0
                    ? `translateY(${viewDrawer.dragY}px)`
                    : 'translateY(0)',
                transition:
                  viewDrawer.isDragging
                    ? 'none'
            ...
            >
              {viewData.image_url ? (
                <div className="relative h-48 sm:h-56 w-full shrink-0 overflow-hidden bg-slate-100 -mb-2 sm:mb-0">
                  <div
                    ...
                    onTouchStart={
                      viewDrawer.handleTouchStart
                    }
                    onTouchMove={
                      viewDrawer.handleTouchMove
                    }
                    onTouchEnd={
                      viewDrawer.handleTouchEnd
                    }
                  >
                  ...
                  <button
                    ...
                    className="hidden sm:flex w-7 h-7 rounded-full items-center justify-center text-slate-400 hover:bg-slate-100 hover:text-slate-600 transition-colors absolute top-4 right-4 z-30"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 20 20"
                ...
                <>
                  ...
                  >
                    <div
                      className="w-12 h-1.5 bg-slate-300 rounded-full transition-colors"
                      style={{
                        background:
                          viewDrawer.isDragging
                            ? '#466460'
                  ...
                  >
                    ...
                    <button
                      ...
                      className="hidden sm:flex w-7 h-7 rounded-full items-center justify-center text-slate-400 hover:bg-slate-100 hover:text-slate-600 transition-colors"
                    >
              ...
              <div className="px-5 sm:px-6 py-5 overflow-y-auto grow scrollbar-none bg-white relative z-10">
                <div className="flex flex-wrap gap-2 mb-3">
                  {viewData.priority &&
                    viewData.priority !==
                      'normal' && (
                      <span
                        className={`text-xs font-bold px-3 py-1 rounded-full text-white ${PRIORITY_CONFIG[
                          viewData
                            .priority
                        ]?.color}`}
                      >
                        ...
                        {displayDepts.map(
                          (
                            ...
                            <span
                              ...
                              className="text-xs font-semibold px-3 py-1 rounded-full bg-[#e0eceb] text-[#466460]"
                            >
                              {d ===
                              ALL_DEPT_LABEL
                                ? d
                ...
                <h3 className="text-xl sm:text-2xl font-bold text-[#466460] mb-2 leading-snug">
                  {viewData.title}
                </h3>
                ...
                <div className="flex flex-col sm:flex-row sm:flex-wrap gap-y-2 sm:gap-y-1.5 gap-x-4 text-[13px] sm:text-sm text-slate-500 mb-4">
                  <span>
                    <i className="fa-regular fa-calendar mr-1 w-3 text-center" />
                    {formatDate(
                      viewData.created_at
                    )}
                  </span>
                  ...
                  {viewData.location && (
                    <span>
                      <i className="fa-solid fa-location-dot mr-1 w-3 text-center text-[#e07a5f]" />
                      {
                        viewData.location
                      }
                    </span>
                  ...
                  {viewData.contact_person && (
                    <span>
                      <i className="fa-solid fa-user mr-1 w-3 text-center text-[#466460]" />
                      {
                        viewData.contact_person
                      }
                    </span>
                  ...
                  {viewData.contact_email && (
                    <span>
                      <i className="fa-solid fa-envelope mr-1 w-3 text-center text-[#466460]" />
                      {
                        viewData.contact_email
                      }
                    </span>
