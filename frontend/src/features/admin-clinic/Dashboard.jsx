...
// (Replace these with your real page components as you build them)
// ============================================================
const PlaceholderView = ({ title, emoji }) => (
  <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-6">
    <div className="text-6xl mb-4">{emoji}</div>
    <h2 className="text-xl font-bold text-[#466460] mb-2">{title}</h2>
...
function DashboardContent() {
  ...
  const clearancesIssued = Math.floor(Math.random() * 20) + 5;
  ...
  const trendData = useMemo(() => {
    ...
    const datasets = types.map(t => {
      const data = labels.map(lbl => {
        ...
        return filteredPeople.filter(p => p.type === t).reduce((sum, p) => {
  ...
  const diseaseChartData = useMemo(() => {
    ...
    return {
      ...
      config: {
        ...
        datasets: [{ data: data.length ? data : [1], backgroundColor: DISEASE_COLORS.slice(0, Math.max(labels.length, 1)), borderWidth: 0 }]
      }
    };
  }, [filteredPeople]);
  ...
  const typeChartData = useMemo(() => {
    ...
    return {
      ...
      datasets: [{ label: 'Count', data: Object.values(counts), backgroundColor: Object.keys(counts).map(t => TYPE_COLORS[t] || '#466460'), borderRadius: 4 }]
    };
  }, [filteredPeople]);

  const alerts = [
    ...(pendingCount > 0 ? [{ icon: 'fa-calendar', text: `${pendingCount} appointments pending approval`, type: 'warning' }] : []),
    { icon: 'fa-syringe', text: 'Vaccination stock running low', type: 'info' },
    { icon: 'fa-file-medical', text: '5 medical clearances expiring soon', type: 'warning' },
  ];
  ...
  return (
    <div className="min-h-[calc(100vh-120px)] bg-[#f4f7f6] px-4 md:px-6 py-4 md:py-6 font-['Inter',sans-serif] text-[#2d3748]">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-5 mb-5 md:mb-6">
        <GlassCard className="p-4 md:p-5">
          ...
          <h4 className="text-2xl md:text-3xl font-bold text-slate-800">{peopleData.length}</h4>
        ...
        <GlassCard className="p-4 md:p-5">
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-wider mb-1">This Week</p>
          <h4 className="text-2xl md:text-3xl font-bold text-slate-800">{weekAppts.length}</h4>
          <p className="text-xs text-emerald-500 mt-1">scheduled</p>
        </GlassCard>
        <GlassCard className="p-4 md:p-5">
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-wider mb-1">Clearances</p>
          <h4 className="text-2xl md:text-3xl font-bold text-slate-800">{clearancesIssued}</h4>
      ...
      <GlassCard className="p-4 md:p-5 mb-5 md:mb-6">
          ...
          <div className="flex gap-2 overflow-x-auto pb-1 -mb-1 [&::-webkit-scrollbar]:hidden">
            {['all', 'student', 'instructor', 'staff'].map(f => (
              <button key={f} onClick={() => setFilter(f)}
                ...
                ${filter === f ? 'bg-gradient-to-br from-[#466460] to-[#5a7a76] text-white border-transparent' : 'bg-[#f1f5f9] text-[#475569] border-[#e2e8f0]'}`}>
        ...
        <div className="mb-5">
            ...
            <div className="flex gap-1.5 overflow-x-auto pb-1 [&::-webkit-scrollbar]:hidden">
              {['all', '2024', '2025', '2026'].map(y => (
                <button key={y} onClick={() => setYearFilter(y)}
                  ...
                  ${yearFilter === y ? 'bg-gradient-to-br from-[#e07a5f] to-[#c96a4f] text-white border-transparent' : 'bg-[#f1f5f9] text-[#475569] border-[#e2e8f0]'}`}>
                  {y === 'all' ? 'All Years' : y}
                </button>
              ))}
            </div>
          ...
          <div className="flex gap-4 mb-3 flex-wrap">
            {(filter === 'all' || filter === 'student') && <div className="flex items-center gap-1.5 text-[11px] text-[#475569]"><span className="w-7 h-[3px] rounded bg-[#466460]"></span>Students</div>}
            {(filter === 'all' || filter === 'instructor') && <div className="flex items-center gap-1.5 text-[11px] text-[#475569]"><span className="w-7 h-[3px] rounded" style={{background:'repeating-linear-gradient(90deg,#e07a5f 0,#e07a5f 6px,transparent 6px,transparent 10px)'}}></span>Faculty</div>}
            {(filter === 'all' || filter === 'staff') && <div className="flex items-center gap-1.5 text-[11px] text-[#475569]"><span className="w-7 h-[3px] rounded" style={{background:'repeating-linear-gradient(90deg,#81b29a 0,#81b29a 3px,transparent 3px,transparent 6px)'}}></span>Staff</div>}
          </div>

          <div className="w-full h-[220px] md:h-[260px] relative">
            <Line data={trendData} options={{ responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { x: { ticks: { font: { size: 10 }, maxRotation: 45, maxTicksLimit: yearFilter === 'all' ? 12 : 8 }, grid: { color: 'rgba(0,0,0,0.04)' } }, y: { beginAtZero: true, title: { display: true, text: 'Records', font: { size: 10, weight: '600' }, color: '#64748b' }, ticks: { font: { size: 10 }, stepSize: 1, callback: val => Number.isInteger(val) ? val : null }, grid: { color: 'rgba(0,0,0,0.04)' } } } }} />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div>
            <p className="text-[10px] font-black text-slate-500 uppercase tracking-wider mb-3">
              <i className="fa-solid fa-virus mr-1 text-[#466460]"></i>Disease Distribution
            </p>
            <div className="h-[180px] flex justify-center items-center">
              <Doughnut data={diseaseChartData.config} options={{ responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'right', labels: { boxWidth: 10, font: { size: 10 } } } } }} />
            </div>
          </div>
          <div>
            <p className="text-[10px] font-black text-slate-500 uppercase tracking-wider mb-3">
              <i className="fa-solid fa-users mr-1 text-[#466460]"></i>Patient Type Breakdown
            </p>
            <div className="h-[180px]">
              <Bar data={typeChartData} options={{ responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true, title: { display: true, text: 'Records', font: { size: 10, weight: '600' }, color: '#64748b' }, ticks: { font: { size: 10 }, stepSize: 1 } }, x: { ticks: { font: { size: 10 } } } } }} />
            </div>
          </div>
        </div>
        ...
        <div className="mt-5 grid grid-cols-3 gap-3 text-center">
          <div className="p-3 bg-slate-50 rounded-lg">
            <p className="text-[10px] font-black text-slate-400 mb-1">Total Visits</p>
            <p className="text-xl md:text-2xl font-bold text-[#466460]">{totalVisits}</p>
          </div>
          <div className="p-3 bg-slate-50 rounded-lg">
            <p className="text-[10px] font-black text-slate-400 mb-1">Top Case</p>
            <p className="text-xs font-bold text-[#466460] mt-2 leading-tight">{diseaseChartData.topDisease}</p>
          </div>
          <div className="p-3 bg-slate-50 rounded-lg">
            <p className="text-[10px] font-black text-slate-400 mb-1">Recovery</p>
            <p className="text-xl md:text-2xl font-bold text-[#466460]">94%</p>
          </div>
        </div>
      </GlassCard>
      ...
      <GlassCard className="p-4 md:p-5 mb-5 md:mb-6">
        <h3 className="font-bold text-sm text-[#466460] mb-4">
          <i className="fa-solid fa-clock-rotate-left mr-2"></i>Visit Trends
        </h3>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 text-center">
          <div className="p-3 bg-slate-50 rounded-lg"><p className="text-[10px] font-black text-slate-400 mb-1">Today</p><p className="text-xl font-bold text-[#466460]">{todayAppts.length}</p></div>
          <div className="p-3 bg-slate-50 rounded-lg"><p className="text-[10px] font-black text-slate-400 mb-1">This Week</p><p className="text-xl font-bold text-[#466460]">{weekAppts.length}</p></div>
          <div className="p-3 bg-slate-50 rounded-lg"><p className="text-[10px] font-black text-slate-400 mb-1">This Month</p><p className="text-xl font-bold text-[#466460]">{appointments.length}</p></div>
          <div className="p-3 bg-slate-50 rounded-lg"><p className="text-[10px] font-black text-slate-400 mb-1">Avg/Day</p><p className="text-xl font-bold text-[#466460]">{Math.round(appointments.length / 30)}</p></div>
        </div>
      </GlassCard>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-5">
        <GlassCard className="p-4 md:p-5">
          <h3 className="font-bold text-sm text-[#466460] mb-4">
            <i className="fa-solid fa-clock-rotate-left mr-2"></i>Recent Activity
          </h3>
          <div className="space-y-2 max-h-[208px] overflow-y-auto pr-2 [&::-webkit-scrollbar]:w-[5px] [&::-webkit-scrollbar-thumb]:bg-[#466460] [&::-webkit-scrollbar-thumb]:rounded-full">
        ...
        <GlassCard className="p-4 md:p-5">
          <h3 className="font-bold text-sm text-[#466460] mb-4">
            <i className="fa-solid fa-bell mr-2"></i>Alerts & Notifications
          </h3>
...
// ============================================================
// TAB → COMPONENT MAP
// Add your real components here as you build them out.
// ============================================================
function renderTab(activeTab) {
  switch (activeTab) {
    case 'dashboard':     return <DashboardContent />;
    case 'records':       return <PlaceholderView title="Medical Records" emoji="📋" />;
    case 'appointments':  return <PlaceholderView title="Appointments" emoji="📅" />;
    case 'consultations': return <PlaceholderView title="Consultations" emoji="💬" />;
    case 'users':         return <PlaceholderView title="User Management" emoji="👥" />;
