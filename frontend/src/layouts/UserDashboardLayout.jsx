...
function MobilePillNav({ activeTab, onTabChange }) {
  ...
  // Pill is 88% of viewport width, clamped between 280 px and 480 px
  ...
  // bottomOffset is the distance from the viewport bottom edge to the pill.
  // We expose this so MobileShell can set paddingBottom = pillH + bottomOffset + extra.
...
function MobileShell({ activeTab, onTabChange, preview, onClosePreview, children, notificationCount, onNotificationClick }) {
  return (
    ...
    >
      ...
      {/* * Adjusted paddingBottom to 88px to perfectly snap the sticky input bar
       * just above the navigation pill without a massive gap.
       * Pill is ~56px + ~20px offset + ~12px gap = 88px
       */}
      <div
        style={{
          flex: 1,
