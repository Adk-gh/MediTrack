...
const getProfile = async (req, res, next) => {
  try {
    ...
    if (!uid) return res.status(400).json({ success: false, message: "User UID not found in token." });
...
const updateProfile = async (req, res, next) => {
  try {
    ...
    // Never allow the client to control academic acknowledgement version.
...
const adminUpdateUser = async (req, res, next) => {
  try {
    ...
    if (!targetUid) return res.status(400).json({ success: false, message: "Target user UID is required" });
